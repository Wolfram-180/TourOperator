object FZai_vvod: TFZai_vvod
  Left = 32
  Top = 165
  BorderIcons = []
  BorderStyle = bsToolWindow
  Caption = #1047#1072#1103#1074#1082#1072' '#1085#1072' '#1072#1074#1090#1086#1073#1091#1089#1085#1099#1081' '#1090#1091#1088
  ClientHeight = 449
  ClientWidth = 961
  Color = 16574936
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDesktopCenter
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 4
    Top = 19
    Width = 49
    Height = 13
    Caption = #1060#1072#1084#1080#1083#1080#1103
  end
  object Label2: TLabel
    Left = 124
    Top = 19
    Width = 22
    Height = 13
    Caption = #1048#1084#1103
  end
  object Label3: TLabel
    Left = 244
    Top = 19
    Width = 47
    Height = 13
    Caption = #1054#1090#1095#1077#1089#1090#1074#1086
  end
  object Label4: TLabel
    Left = 364
    Top = 19
    Width = 68
    Height = 13
    Caption = #8470' '#1076#1086#1082#1091#1084#1077#1085#1090#1072
  end
  object Label5: TLabel
    Left = 484
    Top = 19
    Width = 79
    Height = 13
    Caption = #1044#1072#1090#1072' '#1088#1086#1078#1076#1077#1085#1080#1103
  end
  object Label6: TLabel
    Left = 636
    Top = 19
    Width = 31
    Height = 13
    Caption = #1040#1076#1088#1077#1089
  end
  object Label8: TLabel
    Left = 872
    Top = 19
    Width = 45
    Height = 13
    Caption = #1058#1077#1083#1077#1092#1086#1085
  end
  object Label9: TLabel
    Left = 584
    Top = 19
    Width = 20
    Height = 13
    Caption = #1055#1086#1083
  end
  object Label11: TLabel
    Left = 490
    Top = 59
    Width = 51
    Height = 13
    Caption = #1058#1088#1072#1085#1089#1092#1077#1088
  end
  object Label16: TLabel
    Left = 4
    Top = 59
    Width = 52
    Height = 13
    Caption = #1040#1075#1077#1085#1090#1089#1090#1074#1086
  end
  object Label17: TLabel
    Left = 344
    Top = 60
    Width = 43
    Height = 13
    Caption = #1055#1080#1090#1072#1085#1080#1077
  end
  object Label19: TLabel
    Left = 4
    Top = 95
    Width = 65
    Height = 13
    Caption = #1044#1072#1090#1072' '#1079#1072#1103#1074#1082#1080
  end
  object Label41: TLabel
    Left = 96
    Top = 95
    Width = 66
    Height = 13
    Caption = #1044#1072#1090#1072' '#1086#1087#1083#1072#1090#1099
  end
  object Label18: TLabel
    Left = 490
    Top = 93
    Width = 92
    Height = 13
    Caption = #1058#1086#1095#1082#1072' '#1084#1072#1088#1096#1088#1091#1090#1072' 1'
  end
  object Label20: TLabel
    Left = 620
    Top = 93
    Width = 92
    Height = 13
    Caption = #1058#1086#1095#1082#1072' '#1084#1072#1088#1096#1088#1091#1090#1072' 2'
  end
  object Label21: TLabel
    Left = 752
    Top = 93
    Width = 92
    Height = 13
    Caption = #1058#1086#1095#1082#1072' '#1084#1072#1088#1096#1088#1091#1090#1072' 3'
  end
  object Label22: TLabel
    Left = 186
    Top = 59
    Width = 94
    Height = 13
    Caption = #1050#1072#1090#1077#1075#1086#1088#1080#1103' '#1085#1086#1084#1077#1088#1072
  end
  object Label23: TLabel
    Left = 187
    Top = 95
    Width = 91
    Height = 13
    Caption = #1057#1074#1086#1073#1086#1076#1085#1099#1077' '#1084#1077#1089#1090#1072
  end
  object Label10: TLabel
    Left = 124
    Top = 3
    Width = 9
    Height = 13
    Caption = '---'
  end
  object IDL: TLabel
    Left = 920
    Top = 19
    Width = 11
    Height = 13
    Caption = 'ID'
  end
  object Label7: TLabel
    Left = 40
    Top = 168
    Width = 55
    Height = 13
    Caption = #1057#1090#1086#1080#1084#1086#1089#1090#1100
  end
  object Label12: TLabel
    Left = 75
    Top = 148
    Width = 22
    Height = 13
    Caption = 'SPO'
  end
  object Label13: TLabel
    Left = 4
    Top = 3
    Width = 109
    Height = 13
    Caption = #1054#1073#1098#1077#1082#1090' '#1088#1072#1079#1084#1077#1097#1077#1085#1080#1103':'
  end
  object Label14: TLabel
    Left = 364
    Top = 3
    Width = 34
    Height = 13
    Caption = #1047#1072#1077#1079#1076':'
  end
  object Label15: TLabel
    Left = 408
    Top = 3
    Width = 9
    Height = 13
    Caption = '---'
  end
  object Label24: TLabel
    Left = 557
    Top = 3
    Width = 53
    Height = 13
    Caption = #8470' '#1079#1072#1103#1074#1082#1080':'
  end
  object Label25: TLabel
    Left = 621
    Top = 3
    Width = 9
    Height = 13
    Caption = '---'
  end
  object Label26: TLabel
    Left = 722
    Top = 3
    Width = 58
    Height = 13
    Caption = #1055#1086#1076#1088#1077#1075#1080#1086#1085':'
  end
  object Label27: TLabel
    Left = 786
    Top = 3
    Width = 9
    Height = 13
    Caption = '---'
  end
  object te1: TcxTextEdit
    Left = 4
    Top = 32
    Width = 121
    Height = 21
    TabOrder = 0
  end
  object te2: TcxTextEdit
    Left = 124
    Top = 32
    Width = 121
    Height = 21
    TabOrder = 1
  end
  object te3: TcxTextEdit
    Left = 244
    Top = 32
    Width = 121
    Height = 21
    TabOrder = 2
  end
  object de1: TcxDateEdit
    Left = 484
    Top = 32
    Width = 101
    Height = 21
    EditValue = '01.01.1900'
    Properties.InputKind = ikMask
    TabOrder = 4
  end
  object te4: TcxTextEdit
    Left = 364
    Top = 32
    Width = 121
    Height = 21
    TabOrder = 3
  end
  object te5: TcxTextEdit
    Left = 632
    Top = 32
    Width = 241
    Height = 21
    TabOrder = 6
  end
  object te6: TcxTextEdit
    Left = 872
    Top = 32
    Width = 47
    Height = 21
    TabOrder = 11
  end
  object polcb: TcxComboBox
    Left = 584
    Top = 32
    Width = 49
    Height = 21
    ParentShowHint = False
    Properties.DropDownListStyle = lsFixedList
    Properties.DropDownRows = 20
    Properties.Items.Strings = (
      #1046
      #1052)
    Properties.ReadOnly = False
    Properties.Sorted = True
    ShowHint = False
    TabOrder = 5
    Text = #1052
  end
  object tr1: TdxDBTreeList
    Left = 2
    Top = 226
    Width = 959
    Height = 199
    Bands = <
      item
      end>
    DefaultLayout = True
    HeaderPanelRowCount = 1
    KeyField = 'id'
    ParentField = 'p'
    TabOrder = 12
    OnClick = tr1Click
    DataSource = ds2
    IniFileName = 'grids.ini'
    IniSectionName = 'z_stat'
    LookAndFeel = lfUltraFlat
    OptionsBehavior = [etoAutoDragDrop, etoAutoDragDropCopy, etoAutoSort, etoDragExpand, etoDragScroll, etoEditing, etoEnterShowEditor, etoImmediateEditor, etoStoreToIniFile, etoTabThrough]
    RootValue = '-1'
    TreeLineColor = clGrayText
    object tr1id: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id'
    end
    object katColumn: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1050#1072#1090#1077#1075#1086#1088#1080#1103' '#1085#1086#1084#1077#1088#1072
      Width = 124
      BandIndex = 0
      RowIndex = 0
      FieldName = 'katcol'
    end
    object tr1n: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 20
      BandIndex = 0
      RowIndex = 0
      FieldName = 'n'
    end
    object tr1p: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 20
      BandIndex = 0
      RowIndex = 0
      FieldName = 'p'
    end
    object tr1id_n: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id_n'
    end
    object tr1id_m: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id_m'
    end
    object tr1f: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1060#1072#1084#1080#1083#1080#1103
      Sorted = csUp
      Width = 100
      BandIndex = 0
      RowIndex = 0
      FieldName = 'f'
    end
    object tr1i: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1048#1084#1103
      Width = 85
      BandIndex = 0
      RowIndex = 0
      FieldName = 'i'
    end
    object tr1o: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1054#1090#1095#1077#1089#1090#1074#1086
      Width = 96
      BandIndex = 0
      RowIndex = 0
      FieldName = 'o'
    end
    object StoimCol: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1057#1090#1086#1080#1084#1086#1089#1090#1100
      Width = 124
      BandIndex = 0
      RowIndex = 0
      FieldName = 'stoim'
    end
    object tr1nd: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #8470' '#1076#1086#1082#1091#1084#1077#1085#1090#1072
      Width = 72
      BandIndex = 0
      RowIndex = 0
      FieldName = 'nd'
    end
    object tr1pol: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1055#1086#1083
      Width = 58
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pol'
    end
    object tr1adres: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1040#1076#1088#1077#1089
      Width = 61
      BandIndex = 0
      RowIndex = 0
      FieldName = 'adres'
    end
    object tr1tel: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1058#1077#1083#1077#1092#1086#1085
      Width = 61
      BandIndex = 0
      RowIndex = 0
      FieldName = 'tel'
    end
    object tr1pods: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1055#1086#1076#1089#1077#1083#1077#1085#1080#1077
      Width = 72
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pods'
    end
    object tr1per: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1055#1077#1088#1080#1086#1076
      Visible = False
      Width = 48
      BandIndex = 0
      RowIndex = 0
      FieldName = 'per'
    end
    object tr1nz: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Caption = #8470' '#1079#1072#1103#1074#1082#1080
      Width = 40
      BandIndex = 0
      RowIndex = 0
      FieldName = 'nz'
    end
    object tr1tip: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'tip'
    end
    object tr1id_ag: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id_ag'
    end
    object tr1id_gost: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id_gost'
    end
    object tr1id_tr: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id_tr'
    end
    object tr1id_pit: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id_pit'
    end
    object tr1id_kat: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id_kat'
    end
    object tr1id_po: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id_po'
    end
    object tr1id_pp: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id_pp'
    end
    object tr1id_pv: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Visible = False
      Width = 38
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id_pv'
    end
    object tr1id_pers: TdxDBTreeListMaskColumn
      Alignment = taLeftJustify
      Caption = #1048#1044' '#1087#1077#1088#1089'.'
      Width = 57
      BandIndex = 0
      RowIndex = 0
      FieldName = 'id_pers'
    end
    object tr1dr: TdxDBTreeListDateColumn
      Alignment = taLeftJustify
      Caption = #1044#1072#1090#1072' '#1088#1086#1078#1076#1077#1085#1080#1103
      Width = 86
      BandIndex = 0
      RowIndex = 0
      FieldName = 'dr'
    end
    object AgColumn: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1040#1075#1077#1085#1090#1089#1090#1074#1086
      Width = 124
      BandIndex = 0
      RowIndex = 0
      FieldName = 'agentcol'
    end
    object PitColumn: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1055#1080#1090#1072#1085#1080#1077
      Width = 124
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pitcol'
    end
    object MestoColumn: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1052#1077#1089#1090#1086
      Width = 93
      BandIndex = 0
      RowIndex = 0
      FieldName = 'm'
    end
    object trColumn: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1058#1088#1072#1085#1089#1092#1077#1088
      Width = 124
      BandIndex = 0
      RowIndex = 0
      FieldName = 'trcol'
    end
    object Tm1Column: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1058#1052'1'
      Width = 86
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pocol'
    end
    object Tm2Column: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1058#1052'2'
      Width = 78
      BandIndex = 0
      RowIndex = 0
      FieldName = 'ppcol'
    end
    object Tm3Column: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1058#1052'3'
      Width = 74
      BandIndex = 0
      RowIndex = 0
      FieldName = 'pvcol'
    end
    object BezMestaColumn: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1041#1077#1079' '#1084#1077#1089#1090#1072
      Width = 59
      BandIndex = 0
      RowIndex = 0
      FieldName = 'bezmesta'
    end
    object proezdColumn: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1055#1088#1086#1077#1079#1076
      Width = 86
      BandIndex = 0
      RowIndex = 0
      FieldName = 'proezd'
    end
    object tip_proezdaColumn: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1058#1080#1087
      Width = 79
      BandIndex = 0
      RowIndex = 0
      FieldName = 'tip_proezda'
    end
    object vikupColumn: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1042#1099#1082#1091#1087#1083#1077#1085#1086
      Width = 88
      BandIndex = 0
      RowIndex = 0
      FieldName = 'vikup'
    end
    object OperColumn: TdxDBTreeListColumn
      Alignment = taLeftJustify
      Caption = #1054#1087#1077#1088#1072#1090#1086#1088
      Width = 117
      BandIndex = 0
      RowIndex = 0
      FieldName = 'oper'
    end
  end
  object trcb: TcxComboBox
    Left = 490
    Top = 72
    Width = 262
    Height = 21
    Properties.DropDownRows = 20
    Properties.ReadOnly = False
    Properties.Sorted = True
    Properties.OnChange = trcbPropertiesChange
    TabOrder = 9
  end
  object agc: TcxComboBox
    Left = 4
    Top = 72
    Width = 183
    Height = 21
    Properties.DropDownRows = 30
    Properties.ReadOnly = False
    Properties.OnChange = agcPropertiesChange
    TabOrder = 7
  end
  object rg1: TcxRadioGroup
    Left = 186
    Top = 189
    Width = 159
    Height = 37
    Caption = ' '#1087#1086#1076#1089#1077#1083#1077#1085#1080#1077' '
    ItemIndex = 0
    Properties.Columns = 2
    Properties.Items = <
      item
        Caption = #1085#1077#1090
      end
      item
        Caption = #1076#1072
      end>
    TabOrder = 13
  end
  object rg2: TcxRadioGroup
    Left = 490
    Top = 125
    Width = 263
    Height = 45
    Caption = ' '#1087#1088#1086#1077#1079#1076' '
    ItemIndex = 0
    Properties.Columns = 2
    Properties.Items = <
      item
        Caption = #1075#1086#1088#1086#1076'-'#1073#1072#1079#1072'-'#1075#1086#1088#1086#1076
      end
      item
        Caption = #1075#1086#1088#1086#1076'-'#1073#1072#1079#1072
      end
      item
        Caption = #1073#1072#1079#1072'-'#1075#1086#1088#1086#1076
      end
      item
        Caption = #1073#1077#1079' '#1087#1088#1086#1077#1079#1076#1072
      end>
    TabOrder = 14
    OnClick = rg2Click
  end
  object vmcb: TcxCheckBox
    Left = 182
    Top = 174
    Width = 134
    Height = 21
    Caption = #1074#1099#1082#1091#1087#1083#1077#1085#1085#1086#1077' '#1084#1077#1089#1090#1086
    TabOrder = 15
    OnClick = vmcbClick
  end
  object tm3: TcxComboBox
    Left = 752
    Top = 106
    Width = 133
    Height = 21
    Properties.DropDownListStyle = lsFixedList
    Properties.DropDownRows = 20
    Properties.ReadOnly = False
    Properties.Sorted = True
    TabOrder = 16
  end
  object tm2: TcxComboBox
    Left = 620
    Top = 106
    Width = 133
    Height = 21
    Properties.DropDownListStyle = lsFixedList
    Properties.DropDownRows = 20
    Properties.ReadOnly = False
    Properties.Sorted = True
    TabOrder = 17
  end
  object tm1: TcxComboBox
    Left = 490
    Top = 106
    Width = 131
    Height = 21
    Properties.DropDownListStyle = lsFixedList
    Properties.DropDownRows = 20
    Properties.ReadOnly = False
    Properties.Sorted = True
    TabOrder = 18
  end
  object dzai: TcxDateEdit
    Left = 4
    Top = 107
    Width = 93
    Height = 21
    EditValue = 21916d
    Properties.SaveTime = False
    Properties.ShowTime = False
    Properties.OnChange = dzaiPropertiesChange
    TabOrder = 19
  end
  object dopl: TcxDateEdit
    Left = 96
    Top = 107
    Width = 91
    Height = 21
    EditValue = 21916d
    Properties.SaveTime = False
    Properties.ShowTime = False
    TabOrder = 20
  end
  object garcb: TcxCheckBox
    Left = 0
    Top = 127
    Width = 89
    Height = 21
    Caption = #1075#1072#1088'. '#1087#1080#1089#1100#1084#1086
    TabOrder = 21
    OnClick = garcbClick
  end
  object b2: TcxButton
    Left = 3
    Top = 204
    Width = 92
    Height = 21
    Caption = #1076#1086#1073#1072#1074#1080#1090#1100
    Default = True
    TabOrder = 22
    OnClick = b2Click
    LookAndFeel.Kind = lfUltraFlat
  end
  object katc: TcxComboBox
    Left = 186
    Top = 72
    Width = 159
    Height = 21
    Properties.DropDownRows = 20
    Properties.ReadOnly = False
    Properties.Sorted = True
    Properties.OnChange = katcPropertiesChange
    TabOrder = 23
  end
  object mlb: TcxListBox
    Left = 186
    Top = 107
    Width = 159
    Height = 55
    ItemHeight = 13
    TabOrder = 24
  end
  object pitlb: TcxListBox
    Left = 344
    Top = 72
    Width = 147
    Height = 132
    ItemHeight = 13
    TabOrder = 8
  end
  object b3: TcxButton
    Left = 186
    Top = 426
    Width = 159
    Height = 21
    Caption = #1091#1076#1072#1083#1080#1090#1100
    TabOrder = 25
    OnClick = b3Click
    LookAndFeel.Kind = lfUltraFlat
  end
  object b4: TcxButton
    Left = 345
    Top = 204
    Width = 145
    Height = 21
    Caption = 'X'
    TabOrder = 26
    OnClick = b4Click
    LookAndFeel.Kind = lfUltraFlat
  end
  object b5: TcxButton
    Left = 3
    Top = 426
    Width = 92
    Height = 21
    Caption = #1079#1072#1082#1086#1085#1095#1080#1090#1100
    TabOrder = 27
    OnClick = b5Click
    LookAndFeel.Kind = lfUltraFlat
  end
  object bmcb: TcxCheckBox
    Left = 182
    Top = 160
    Width = 134
    Height = 21
    Caption = #1073#1077#1079' '#1084#1077#1089#1090#1072
    TabOrder = 28
    OnClick = bmcbClick
  end
  object b6: TcxButton
    Left = 96
    Top = 204
    Width = 90
    Height = 21
    Caption = #1080#1079#1084#1077#1085#1080#1090#1100
    TabOrder = 29
    OnClick = b6Click
    LookAndFeel.Kind = lfUltraFlat
  end
  object b7: TcxButton
    Left = 919
    Top = 53
    Width = 40
    Height = 21
    Caption = 'V'
    TabOrder = 30
    OnClick = b7Click
    LookAndFeel.Kind = lfUltraFlat
  end
  object idte: TcxTextEdit
    Left = 918
    Top = 32
    Width = 42
    Height = 21
    TabOrder = 31
  end
  object b9: TcxButton
    Left = 3
    Top = 182
    Width = 92
    Height = 21
    Caption = #1079#1072#1085#1103#1090#1100
    TabOrder = 32
    OnClick = b9Click
    LookAndFeel.Kind = lfUltraFlat
  end
  object b8: TcxButton
    Left = 96
    Top = 182
    Width = 90
    Height = 21
    Caption = #1086#1089#1074#1086#1073#1086#1076#1080#1090#1100
    TabOrder = 33
    OnClick = b8Click
    LookAndFeel.Kind = lfUltraFlat
  end
  object stoimte: TcxTextEdit
    Left = 99
    Top = 161
    Width = 53
    Height = 21
    TabOrder = 10
    Text = '0'
    OnKeyPress = stoimteKeyPress
  end
  object rg3: TcxRadioGroup
    Left = 490
    Top = 169
    Width = 264
    Height = 57
    Caption = ' '#1090#1080#1087' '
    ItemIndex = 0
    Properties.Columns = 2
    Properties.Items = <
      item
        Caption = #1074#1079#1088#1086#1089#1083#1099#1081
      end
      item
        Caption = #1088#1077#1073#1077#1085#1086#1082' 5-12'
      end
      item
        Caption = #1088#1077#1073#1077#1085#1086#1082' '#1076#1086' 5'
      end
      item
        Caption = #1090#1086#1083#1100#1082#1086' '#1087#1088#1086#1077#1079#1076
      end>
    TabOrder = 34
    OnClick = rg3Click
  end
  object spote: TcxTextEdit
    Left = 99
    Top = 141
    Width = 88
    Height = 21
    TabOrder = 35
  end
  object b10: TcxButton
    Left = 152
    Top = 162
    Width = 34
    Height = 19
    Caption = '<>'
    TabOrder = 36
    OnClick = b10Click
    LookAndFeel.Kind = lfUltraFlat
  end
  object StaticText1: TStaticText
    Left = 614
    Top = 155
    Width = 10
    Height = 10
    AutoSize = False
    Color = 7500529
    ParentColor = False
    TabOrder = 37
  end
  object StaticText2: TStaticText
    Left = 708
    Top = 155
    Width = 10
    Height = 10
    AutoSize = False
    Color = 7500529
    ParentColor = False
    TabOrder = 38
  end
  object q1: TADOQuery
    Connection = DM.ac
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select t1.podreg,t2.podreg,t3.podreg from tr t left join podreg ' +
        't1 on(t1.codeid=t.p1) left join podreg t2 on(t2.codeid=t.p2) lef' +
        't join podreg t3 on(t3.codeid=t.p3) where t.codeid=75')
    Left = 352
    Top = 79
  end
  object q2: TADOQuery
    Connection = DM.ac
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select a.*,b.m from clients a full outer join gostnom b on a.id_' +
        'm=b.codeid where a.nz=1')
    Left = 40
    Top = 247
    object q2id: TAutoIncField
      FieldName = 'id'
      ReadOnly = True
    end
    object q2f: TWideStringField
      FieldName = 'f'
      Size = 50
    end
    object q2i: TWideStringField
      FieldName = 'i'
      Size = 50
    end
    object q2o: TWideStringField
      FieldName = 'o'
      Size = 50
    end
    object q2nd: TWideStringField
      FieldName = 'nd'
      Size = 50
    end
    object q2pol: TWideStringField
      FieldName = 'pol'
      Size = 50
    end
    object q2adres: TWideStringField
      FieldName = 'adres'
      Size = 50
    end
    object q2tel: TWideStringField
      FieldName = 'tel'
      Size = 50
    end
    object q2pods: TWideStringField
      FieldName = 'pods'
      Size = 50
    end
    object q2per: TWideStringField
      FieldName = 'per'
      Size = 50
    end
    object q2vikup: TWideStringField
      FieldName = 'vikup'
      Size = 50
    end
    object q2gar_pis: TWideStringField
      FieldName = 'gar_pis'
      Size = 50
    end
    object q2bezmesta: TWideStringField
      FieldName = 'bezmesta'
      Size = 50
    end
    object q2proezd: TWideStringField
      FieldName = 'proezd'
      Size = 50
    end
    object q2tip_proezda: TWideStringField
      FieldName = 'tip_proezda'
      Size = 50
    end
    object q2nz: TIntegerField
      FieldName = 'nz'
    end
    object q2n: TIntegerField
      FieldName = 'n'
    end
    object q2p: TIntegerField
      FieldName = 'p'
    end
    object q2tip: TIntegerField
      FieldName = 'tip'
    end
    object q2id_ag: TIntegerField
      FieldName = 'id_ag'
    end
    object q2id_gost: TIntegerField
      FieldName = 'id_gost'
    end
    object q2id_tr: TIntegerField
      FieldName = 'id_tr'
    end
    object q2id_pit: TIntegerField
      FieldName = 'id_pit'
    end
    object q2id_n: TIntegerField
      FieldName = 'id_n'
    end
    object q2id_m: TIntegerField
      FieldName = 'id_m'
    end
    object q2id_kat: TIntegerField
      FieldName = 'id_kat'
    end
    object q2id_po: TIntegerField
      FieldName = 'id_po'
    end
    object q2id_pp: TIntegerField
      FieldName = 'id_pp'
    end
    object q2id_pv: TIntegerField
      FieldName = 'id_pv'
    end
    object q2id_pers: TIntegerField
      FieldName = 'id_pers'
    end
    object q2dr: TDateTimeField
      FieldName = 'dr'
    end
    object q2data_zai: TDateTimeField
      FieldName = 'data_zai'
    end
    object q2data_opl: TDateTimeField
      FieldName = 'data_opl'
    end
    object q2agentcol: TStringField
      FieldKind = fkLookup
      FieldName = 'agentcol'
      LookupDataSet = DM.AgentQ
      LookupKeyFields = 'Codeid'
      LookupResultField = 'Naimen'
      KeyFields = 'id_ag'
      Size = 200
      Lookup = True
    end
    object q2trcol: TStringField
      FieldKind = fkLookup
      FieldName = 'trcol'
      LookupDataSet = DM.TransQ
      LookupKeyFields = 'Codeid'
      LookupResultField = 'trstr'
      KeyFields = 'id_tr'
      Size = 200
      Lookup = True
    end
    object q2pitcol: TStringField
      FieldKind = fkLookup
      FieldName = 'pitcol'
      LookupDataSet = DM.Pit
      LookupKeyFields = 'Codeid'
      LookupResultField = 'Pit'
      KeyFields = 'id_pit'
      Size = 200
      Lookup = True
    end
    object q2katcol: TStringField
      FieldKind = fkLookup
      FieldName = 'katcol'
      LookupDataSet = DM.Kat
      LookupKeyFields = 'Codeid'
      LookupResultField = 'Kat'
      KeyFields = 'id_kat'
      Size = 200
      Lookup = True
    end
    object q2pocol: TStringField
      FieldKind = fkLookup
      FieldName = 'pocol'
      LookupDataSet = DM.PodRegQ
      LookupKeyFields = 'Codeid'
      LookupResultField = 'Podreg'
      KeyFields = 'id_po'
      Size = 200
      Lookup = True
    end
    object q2ppcol: TStringField
      FieldKind = fkLookup
      FieldName = 'ppcol'
      LookupDataSet = DM.PodRegQ
      LookupKeyFields = 'Codeid'
      LookupResultField = 'Podreg'
      KeyFields = 'id_pp'
      Size = 200
      Lookup = True
    end
    object q2pvcol: TStringField
      FieldKind = fkLookup
      FieldName = 'pvcol'
      LookupDataSet = DM.PodRegQ
      LookupKeyFields = 'Codeid'
      LookupResultField = 'Podreg'
      KeyFields = 'id_pv'
      Size = 200
      Lookup = True
    end
    object q2a_b: TWideStringField
      FieldName = 'a_b'
      Size = 50
    end
    object q2m: TWideStringField
      FieldName = 'm'
      Size = 200
    end
    object q2annul: TWideStringField
      FieldName = 'annul'
      Size = 4
    end
    object q2stoim: TBCDField
      FieldName = 'stoim'
      Precision = 19
    end
    object q2oper: TWideStringField
      FieldName = 'oper'
      Size = 50
    end
    object q2oplata: TWideStringField
      FieldName = 'oplata'
      Size = 50
    end
    object q2podtv: TWideStringField
      FieldName = 'podtv'
      Size = 50
    end
    object q2bez_pr: TWideStringField
      FieldName = 'bez_pr'
      Size = 5
    end
    object q2spo: TWideStringField
      FieldName = 'spo'
      Size = 50
    end
    object q2n_vauch: TIntegerField
      FieldName = 'n_vauch'
    end
    object q2data_podtv: TDateTimeField
      FieldName = 'data_podtv'
    end
    object q2data_vauch: TDateTimeField
      FieldName = 'data_vauch'
    end
    object q2oplacheno: TBCDField
      FieldName = 'oplacheno'
      Precision = 19
    end
    object q2sebest: TBCDField
      FieldName = 'sebest'
      Precision = 19
    end
  end
  object ds2: TDataSource
    DataSet = q2
    Left = 87
    Top = 247
  end
end
